...
//                   className="border-b border-gray-200 hover:bg-gray-200 cursor-pointer transition relative"
//                 >
...
// Reusable Components
const Section = ({ title, icon: Icon, children, status }) => (
  <div className="bg-white rounded-xl border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow relative">
    {status !== undefined && (
      ...
      } text-xs px-2 py-1 rounded-full font-medium shadow z-10`}>
...
const ApprovalRejectedComponent = () => {
  ...
  const fetchNotApproved = async (page = 1) => {
    try {
      const response = await axios.get(`/approval/notApproved?page=${page}`);
      const { data, totalPages } = response.data;
      console.log("response data from fetchNotApproved", response.data);
      setPendingApproval(data);
      setFilterData(data);
  ...
  const handleClickApproved = async (item) => {
    ...
    setApprovalData(response.data.data);
    console.log("data form handleClickApproval", response);
  };
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-gradient-to-br from-[#fdf5f6] via-white to-[#f5eff0]">
      ...
      <div className="fixed inset-0 -z-10 overflow-hidden pointer-events-none">
        <div className="absolute top-0 right-0 w-96 h-96 bg-gradient-to-br from-red-500/5 to-transparent rounded-full blur-3xl"></div>
      ...
      <div className="bg-white/80 backdrop-blur-sm border-b border-gray-200 shadow-sm">
        <div className="px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Rejected Applications</h1>
              <p className="text-sm text-gray-600 mt-0.5">View all rejected student applications</p>
            </div>
            <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-red-600 to-red-700 rounded-lg shadow-lg">
              <XCircle className="w-4 h-4 text-white" />
              <span className="text-white font-semibold text-sm">{filterData.length} Rejected</span>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mb-6">
            ...
            <div className="relative">
              ...
              <input
                type="text"
                className="w-full pl-10 pr-4 py-3 bg-white border border-gray-200 rounded-xl focus:ring-2 focus:ring-red-500/20 focus:border-red-500 transition-all text-gray-900 placeholder-gray-400"
          ...
          {filterData?.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-lg p-12 text-center border border-gray-100">
              ...
              <p className="text-gray-600">There are no rejected applications yet</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl shadow-lg overflow-hidden border border-gray-100">
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-t border-gray-100 bg-gray-50">
                  <button
                    onClick={() => filterByAckNumber ? filter(currentPage - 1) : fetchNotApproved(currentPage - 1)}
                    disabled={currentPage === 1}
      ...
      {showPopup && popupData && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
            {/* Modal Header */}
            <div className="sticky top-0 bg-gradient-to-r from-red-600 to-red-700 px-6 py-4 flex items-center justify-between border-b border-white/10 z-10">
              <div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <Section 
                ...
                icon={User}
                status={approvalData?.studentDetails?.status}
              >
                <InfoGrid>
              ...
              <Section 
                ...
                icon={User}
                status={approvalData?.parentDetails?.status}
              >
                <InfoGrid>
              ...
              <Section 
                ...
                icon={MapPin}
                status={approvalData?.addressDetails?.status}
              >
                <InfoGrid>
              ...
              <Section 
                title="Documents" 
                icon={Image}
                status={approvalData?.documentsDetails?.status}
              >
              ...
              <Section 
                title="Signatures" 
                icon={PenTool}
                status={approvalData?.signatureDetails?.status}
              >
